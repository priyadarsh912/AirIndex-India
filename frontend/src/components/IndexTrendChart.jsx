  ...
  return (
    <div id="daily-trend-section" className="bg-surface-card rounded-xl p-6 shadow-sm border border-border-hairline relative scroll-mt-44">
      ...
      {isLoading && (!trendData || trendData.length === 0 ? (
        ...
        <div className="absolute top-4 right-4 z-20 flex items-center gap-2 bg-surface-card/90 backdrop-blur-sm px-3 py-1.5 rounded-full border border-border-hairline shadow-sm animate-pulse">
          <div className="w-3.5 h-3.5 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
          <span className="text-[11px] font-medium text-text-muted">Updating Index...</span>
        </div>
      ))}
